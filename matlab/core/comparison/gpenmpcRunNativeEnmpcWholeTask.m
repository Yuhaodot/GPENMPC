...
if numel(legIds) ~= expectedLegs
    error("gpenmpcRunNativeEnmpcWholeTask:LegCount", ...
        "Reference and task-plan leg counts differ.");
end
...
for legCursor = 1:numel(legIds)
    ...
    [duration, payloadAfter, groundDuration] = ...
        serviceAccounting(planLeg, payloadKg);
    energy = 0.0;
    if duration > 0.0
        ...
        [state, service] = gpenmpcApplyFixedC3ServiceTransition( ...
            state, payloadKg, payloadAfter, duration, groundDuration, ...
            serviceNode, assets.profile);
...
result = computeResult(trace, task, methodId, config, solver, ...
    ...
    allocation.per_rotor_upper_n, allocation.maximum_tilt_rad);
end
...
function [duration, payloadAfter, groundDuration] = ...
        serviceAccounting(planLeg, payloadBefore)
duration = 0.0; payloadAfter = payloadBefore; groundDuration = 0.0;
...
if isfield(service, "ground_service_duration_s")
    groundDuration = double(service.ground_service_duration_s);
end
end
...
result.all_threshold_checks_pass = all(cell2mat(struct2cell(result.screens)));
end
