#include "GPENMPCFullInnerIngress_stdarray.hpp"
#include <algorithm>
#include <cstdio>
#include <cstring>
#include <vector>
...
// Host-only destination shape for testing the mapping function independently
// of generated uORB types and broker execution.
struct TopicShape {
    ...
    std::uint8_t mavlink_sequence{},wire_payload_length{};std::uint16_t payload_type{};
    std::uint8_t payload_length{},payload[128]{};
...
int main(int argc,char **argv){
    ...
    TopicShape topic{};copy_to_topic(first,receiver.counters(),topic);
    check(topic.timestamp==first.timestamp&&topic.reception_sequence==first.reception_sequence&&topic.receiver_instance==3
        ...
        &&topic.payload_length==128&&std::equal(first.payload.begin(),first.payload.end(),topic.payload),"all topic fields copied");
    ...
    std::printf("{\"checks\":%u,\"failures\":%u,\"payload_type\":%u,\"queue_capacity\":%zu,\"real_generated_mavlink\":true,\"uorb_broker_executed\":false,\"board_actions\":0}\n",checks,failures,payload_type,queue_capacity);
    if(argc>1){
        FILE *file=std::fopen(argv[1],"wb");if(!file){std::fprintf(stderr,"Cannot write test result\n");return 2;}
        std::fprintf(file,"{\"checks\":%u,\"failures\":%u,\"payload_type\":%u,\"queue_capacity\":%zu,\"source_ids_are_authentication\":false,\"real_generated_mavlink\":true,\"uorb_broker_executed\":false,\"wire_delivery_proven\":false,\"board_actions\":0,\"cases\":[",checks,failures,payload_type,queue_capacity);
