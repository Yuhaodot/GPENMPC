...
struct Counters {
    std::uint64_t outcomes[static_cast<std::size_t>(Result::Count)]{};
...
public:
    Result receive(const mavlink_message_t &frame, std::uint64_t ingress_hrt_us,
        ...
        if(sequence_==UINT64_MAX) return record(Result::SequenceExhausted,ingress_hrt_us);
