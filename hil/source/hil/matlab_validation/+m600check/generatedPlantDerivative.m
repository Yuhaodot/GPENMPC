function [derivative, diagnostic] = generatedPlantDerivative( ...
        plantState, rotorCommandN, reference, payloadKg, actualWindXyMps, ...
        globalTimeS, mission, calibration, profile)
%#codegen
% Arithmetic body of immutable gpenmpcM600SixDofPlantDerivative, SHA 9B493F...
% Only functional substitution is the struct-layout-compatible allocation.
gravity = 9.80665;
x = double(plantState(:));
command = double(rotorCommandN(:));
allocation = m600check.generatedAllocation(calibration);
