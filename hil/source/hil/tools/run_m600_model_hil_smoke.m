function result = run_m600_model_hil_smoke(outputDir)
% RUN_M600_MODEL_HIL_SMOKE Run the CopterSim/Pixhawk M600 smoke test.
% The caller verifies the loaded DLL. CopterSim owns the plant and COM;
% MATLAB uses MAVLink UDP. Physical PWM functions remain zero.
...
try
    ...
    while toc(runClock) - flightStart < 20.0
        sendSetpoint(target);
        sendHeartbeatIfDue();
        sampleTelemetry('M600_MODEL_HOVER');
        pause(0.05);
    end
...
hoverRows = rows(strcmp({rows.phase}, 'M600_MODEL_HOVER'));
...
result = struct( ...
    'schema', 'GPENMPC_EFFECTIVE_M600_MODEL_HIL_RESULT_V1', ...
    'status', ternary(passed, ...
        'PASS_EFFECTIVE_M600_COPTERSIM_PIXHAWK_HIL', ...
        'FAIL_EFFECTIVE_M600_COPTERSIM_PIXHAWK_HIL'), ...
    'claim_limit', 'EFFECTIVE_M600_MODEL_HIL', ...
    'topology', struct('plant', 'CopterSim_GPENMPC_M600_CopterSim_DLL', ...
