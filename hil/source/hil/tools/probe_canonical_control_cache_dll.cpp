...
int wmain(int argc,wchar_t**argv){
    ...
    try{File result(argv[3]),csv(argv[4]),cache_raw(argv[5]),rotor_raw(argv[6]),original_raw(argv[7]);
        ...
        std::fprintf(result.f,"],\"cache_packets\":%u,\"rotor_packets\":%u,\"all_zero_cache_packets\":%u,\"first_zero_consumed_tick\":%u,\"original_152_outputs_bit_equal\":%s,\"candidate_DLL_SHA256\":\"%s\",\"parent_DLL_SHA256\":\"%s\",\"localhost_port\":%u,\"model_lifetimes\":%u,\"max_concurrent_models\":%u,\"all_resources_released\":%s,\"claim\":\"Generated-DLL observation probe\"}\n",
