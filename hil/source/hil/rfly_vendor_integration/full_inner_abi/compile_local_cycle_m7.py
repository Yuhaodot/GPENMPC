"""Compile the local execution cycle with Cortex-M7/NuttX flags."""
...
tracked=[vi/'px4_runtime/CanonicalLocalExecutionCycle.hpp',vi/'px4_runtime/CanonicalLocalExecutionCycle.cpp',
 ...
 root/'probe_cycle_size.cpp',Path(__file__)]
def hashes():return {str(p):hashlib.sha256(p.read_bytes()).hexdigest().upper() for p in tracked}
report=dict(scope='ACTUAL_CYCLE_NUTTX_M7_FULL_TABLE312_TU_AND_SIZE_PROBE',source_sha256=hashes(),records=[],
...
try:
 ...
 for name,source in [('cycle',vi/'px4_runtime/CanonicalLocalExecutionCycle.cpp'),('sizes',root/'probe_cycle_size.cpp')]:
