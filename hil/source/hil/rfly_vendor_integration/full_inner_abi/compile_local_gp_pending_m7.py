"""Compile the pending GP interface with Cortex-M7/NuttX flags."""
...
tracked=[vi/'px4_runtime/CanonicalLocalExecutionCycle.hpp',vi/'px4_runtime/CanonicalLocalExecutionCycle.cpp',
 vi/'px4_runtime/Px4CanonicalLocalIo.hpp',vi/'px4_runtime/Px4CanonicalLocalIo.cpp',
 vi/'local_source_ingress/Px4OriginalHilReceiptReader.hpp',vi/'local_source_ingress/Px4OriginalHilReceiptReader.cpp',
 vi/'local_phase/CanonicalLocalPhaseClock.hpp',vi/'local_phase/CanonicalLocalPhase.h',
 vi.parent/'px4_full_inner/px4_state_adapter/AtomicOdometryAdapter.hpp',
 app/'uORB/topics/gpenmpc_original_hil_receipt.h',app/'uORB/topics/uORBTopics.hpp',
 root/'probe_gp_pending_m7.cpp',root/'probe_gp_wire_m7.cpp',vi/'px4_runtime/CanonicalLocalGpPending.hpp',vi/'px4_wire/CanonicalLocalGpWire.hpp',Path(__file__)]
def hashes():return {str(p):hashlib.sha256(p.read_bytes()).hexdigest().upper() for p in tracked}
report=dict(scope='ACTUAL_GP_PENDING_AND_WIRE_NUTTX_M7_TWO_TU',source_sha256=hashes(),records=[],
 linked=False,executed=False,private74_recompiled=False)
try:
 ...
 for name,source in [('pending',root/'probe_gp_pending_m7.cpp'),('wire',root/'probe_gp_wire_m7.cpp')]:
