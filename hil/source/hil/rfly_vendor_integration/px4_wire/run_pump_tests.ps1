...
& $cc -std=c++14 -O2 -ffp-contract=off -fno-fast-math -Wall -Wextra -Werror -pedantic -static -Wno-address-of-packed-member -isystem (Join-Path $rflyHeaders 'mavlink/common') -isystem (Join-Path $rflyHeaders 'mavlink') -I (Join-Path $PSScriptRoot 'pump_host_stub') -I $actual -I $rflyHeaders -I $rflyArm (Join-Path $PSScriptRoot 'test_exchange_pump.cpp') (Join-Path $rflyDir 'px4_runtime/CanonicalExchangePump.cpp') (Join-Path $rflyDir 'px4_runtime/SnapshotOutbox.cpp') (Join-Path $rflyDir 'px4_runtime/Px4CanonicalIo.cpp') @rflyObjects -o test_exchange_pump.exe
if($LASTEXITCODE -ne 0){throw 'compile failed'}
& '.\test_exchange_pump.exe' $rflyFixtures[0] $rflyFixtures[1]
