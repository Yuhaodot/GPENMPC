...
foreach($rflyName in @('CanonicalRflyExecutor.hpp','RflySnapshotBoundExecutor.hpp','SimulinkCanonicalPolicy.hpp','SlimKernelCodec.hpp','SlimSnapshotExecutor.hpp','test_slim_kernel_codec.cpp')){$rflyProtected+=Join-Path $PSScriptRoot $rflyName}
...
try{
    ...
    & (Join-Path $rflyTools 'clang++.exe') -std=c++14 -O2 -ffp-contract=off -fno-fast-math -Wall -Wextra -Werror -pedantic -static -Wno-address-of-packed-member -isystem (Join-Path $rflyMav 'common') -isystem $rflyMav -I (Join-Path $rflyBuild 'px4_full_inner/px4_ingress/generated') -I (Join-Path $vi 'px4_wire/pump_host_stub') -I $rflyHeaders -I $rflyArm (Join-Path $vi 'test_slim_kernel_codec.cpp') @rflyObjects -o test_slim_kernel_codec.exe
    if($LASTEXITCODE -ne 0){throw 'Slim codec HOST compile failed'}
    & '.\test_slim_kernel_codec.exe' @rflyFixtures $rflyConfig $rflyScience $rflyCode $rflyWrapper
