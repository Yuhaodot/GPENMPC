...
ModuleContext *GPENMPCRflyCanonicalModule::configured_context_{nullptr};
ModuleConfiguration GPENMPCRflyCanonicalModule::frozen_configuration_{};
px4::atomic<std::uint8_t> GPENMPCRflyCanonicalModule::phase_{static_cast<std::uint8_t>(ModulePhase::Stopped)};
px4::atomic<std::uint8_t> GPENMPCRflyCanonicalModule::stop_reason_{static_cast<std::uint8_t>(ModuleStopReason::None)};
px4::atomic<std::uint8_t> GPENMPCRflyCanonicalModule::detach_{static_cast<std::uint8_t>(ModuleDetach::Unavailable)};
px4::atomic<std::uint8_t> GPENMPCRflyCanonicalModule::plant_{static_cast<std::uint8_t>(ModulePlantDisposition::Unknown)};

GPENMPCRflyCanonicalModule::GPENMPCRflyCanonicalModule(ModuleContext &context,
...
bool GPENMPCRflyCanonicalModule::configure(ModuleContext *context) noexcept
{
...
bool GPENMPCRflyCanonicalModule::valid_configuration(const ModuleConfiguration &configuration) noexcept
{
...
int GPENMPCRflyCanonicalModule::task_spawn(int argc, char *argv[])
{
    ...
    auto *candidate=new GPENMPCRflyCanonicalModule(*configured_context_,frozen_configuration_);
    if(!candidate){(void)configured_context_->abort_acquire();return -ENOMEM;}
    if(candidate->io_.diagnostics().fault!=Fault::None){delete candidate;(void)configured_context_->abort_acquire();return -EINVAL;}
    ...
    _task_id=px4_task_spawn_cmd("gpenmpc_rfly_canonical",SCHED_DEFAULT,
...
GPENMPCRflyCanonicalModule *GPENMPCRflyCanonicalModule::instantiate(int argc, char *argv[])
{
...
void GPENMPCRflyCanonicalModule::run()
{
    ...
    while(reason==ModuleStopReason::None && !should_exit()){
        const ModulePoll result=context_.poll(io_,hrt_absolute_time());
        if(io_.diagnostics().fault!=Fault::None)reason=ModuleStopReason::IoFault;
...
int GPENMPCRflyCanonicalModule::cooperative_stop()
{
...
int GPENMPCRflyCanonicalModule::persistent_status()
{
    if(pthread_mutex_lock(&px4_modules_mutex)!=0)return -EIO;
...
int GPENMPCRflyCanonicalModule::print_status()
{
...
int GPENMPCRflyCanonicalModule::main(int argc, char *argv[])
{
...
int GPENMPCRflyCanonicalModule::custom_command(int argc, char *argv[])
{
    (void)argc;(void)argv;return print_usage("unrecognized command");
}

int GPENMPCRflyCanonicalModule::print_usage(const char *reason)
{
    if(reason)PX4_WARN("%s",reason);
    PRINT_MODULE_USAGE_NAME("gpenmpc_rfly_canonical","controller");
...
{
    return gpenmpc_rfly_px4::GPENMPCRflyCanonicalModule::main(argc,argv);
}
