...
$reader=Join-Path $vi 'local_source_ingress\Px4OriginalHilReceiptReader.cpp';$test=Join-Path $vi 'test_local_gp_pending.cpp'
$tracked=@($io,$cycle,$reader,$test,$PSCommandPath,$facade,$archive,$phase,$rwi,$sjc,$gp,
 (Join-Path $vi 'px4_runtime\CanonicalLocalExecutionCycle.hpp'),(Join-Path $vi 'px4_runtime\CanonicalLocalGpPending.hpp'),(Join-Path $vi 'px4_wire\CanonicalLocalGpWire.hpp'),(Join-Path $vi 'test_local_execution_cycle.cpp'),(Join-Path $vi 'px4_runtime\Px4CanonicalLocalIo.hpp'),
 (Join-Path $vi 'local_phase\CanonicalLocalPhaseClock.hpp'),(Join-Path $vi 'CanonicalFullInnerConsumption.hpp'),
 ...
 @(Get-ChildItem -LiteralPath (Join-Path $vi 'px4_wire\pump_host_stub') -Recurse -File|ForEach-Object FullName)
...
$receipt=[ordered]@{scope='ACTUAL_GP_PENDING_SAME_CYCLE_PRIVATE74_GP_MAVLINK_CODEC_MOCK_ARRIVALS';
 records=$records;link_command=$linkArgs;link_exit=$le;link_output=$link;run_exit=$rc;run_output=$run;result=$result;
 source_before=$before;source_stable=$stable;
