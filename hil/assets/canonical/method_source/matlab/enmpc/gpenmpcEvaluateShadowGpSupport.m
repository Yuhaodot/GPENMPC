...
% The 1e-12 term is only a floating-point boundary tolerance. The scientific
% interval remains the model's already calibrated half-width.
...
if numel(candidate) ~= config.decision_dimension || any(~isfinite(candidate))
    error("gpenmpcEvaluateShadowGpSupport:Candidate", ...
        "The shadow query requires one finite frozen-dimension candidate.");
end
